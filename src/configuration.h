...
// The parameter _SMART_RECOVERY_ must be set to 1 if you want the 
// Smart Recovery functions enabled for the Minilink messages.
// In case of CRC error, if the message is at least XX bytes long,
// the Smart Recovery functions will try to reassemble the message
// using info from previous messages. 
#define _SMART_RECOVERY_ 1

// The parameter _MDRIP_ENABLED_ must be set to 1 if you want the 
// MMCommander to be able to communicate with a HM-10 module
// emulating a Dexcom G4 Transmitter. (mDrip = xDrip for Medtronic)
#define _MDRIP_ENABLED_ 1 

// The version number is defined here for identification purposes.
#define _MMCOMMANDER_VERSION_ 90
